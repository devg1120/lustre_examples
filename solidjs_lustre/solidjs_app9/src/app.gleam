...
// ⭕ JavaScript側（src/theme_helper.js）で定義したクラス操作関数をGleamに繋ぐ（FFI）
...
fn update(model: Model, msg: Msg) -> Model {
  case msg {
    ...
    ToggleTheme -> {
      ...
      // <html> タグのクラスを JavaScript の FFI 経由で直接操作
      case next_theme {
        DarkTheme -> toggle_html_theme_ffi(True)
        LightTheme -> toggle_html_theme_ffi(False)
      }
...
// --- JSON DECODER ---
// ⭕ 最新の Gleam 標準である `use` 構文を用いたイベントデコーダに修正
...
pub fn view(model: Model) -> Element(Msg) {
  html.div([class("min-h-screen font-sans flex flex-col bg-gray-50 dark:bg-gray-950 text-gray-900 dark:text-gray-100 transition-colors duration-300")], [
    ...
    // メインビューポート（ヘッダー分の高さ 16 を確保）
    html.div([class("pt-16 flex flex-1 h-[calc(100vh-4rem)]")], [
      // 2. 開閉式サイドバー (Toggleable Sidebar)
      html.aside(
        [
          class(
            "bg-gray-100 dark:bg-gray-900 border-r border-gray-200 dark:border-gray-800 text-gray-800 dark:text-gray-200 flex flex-col transition-all duration-300 ease-in-out "
            ...
            html.button(
              [
                class(case model.current_page {
                  ...
                  _ -> "w-full text-left px-4 py-2.5 rounded-lg text-gray-600 dark:text-gray-400 hover:bg-gray-200 dark:hover:bg-gray-800 hover:text-gray-900 dark:hover:text-white transition-all"
                }),
        ...
        case model.current_page {
          DashboardPage ->
            html.div([], [
              ...
              // 3-b. リサイズ可能な分割パネル（SplitPanel）
              html.div([class("mt-8")], [
                html.h3([class("text-lg font-bold text-gray-700 dark:text-gray-300 mb-4")], [
                  element.text("ワークスペース分割プレビュー"),
                ]),
                ...
                element.element(
                  ...
                  [
                    // 左側のスロットコンテンツ
                    html.div([attribute.attribute("slot", "left"), class("space-y-3")], [
                      html.h4([class("font-bold text-blue-600 dark:text-blue-400 text-base")], [element.text("← 左側ワークエリア")]),
                      html.p([class("text-sm text-gray-500 dark:text-gray-400 leading-relaxed")], [
                        element.text("この文章構造やボタンなどの状態ロジックは、すべて Gleam (Lustre) 側で管理されています。"),
                      ]),
                    ]),

                    // 右側のスロットコンテンツ
                    html.div([attribute.attribute("slot", "right"), class("space-y-3")], [
                      html.h4([class("font-bold text-emerald-600 dark:text-emerald-400 text-base")], [element.text("右側データエリア →")]),
                      html.p([class("text-sm text-gray-500 dark:text-gray-400 leading-relaxed")], [
                        element.text("中央の縦バーをつまんでマウスドラッグすることで、ピクセル単位のレイアウト変更が SolidJS 側で超高速に行われます。"),
                      ]),
                    ]),
                  ],
                ),
